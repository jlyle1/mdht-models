...
public class BodyTemperatureOperations extends ClinicalStatementOperations {
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_BODY_TEMPERATURE_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.templateId->exists(id : datatypes::II | id.root = '2.16.840.1.1133883.17.3.10.1.176')";
	...
	protected static Constraint VALIDATE_BODY_TEMPERATURE_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV;
	...
	public static  boolean validateBodyTemperatureTemplateId(BodyTemperature bodyTemperature, DiagnosticChain diagnostics, Map<Object, Object> context) {
		...
		if (VALIDATE_BODY_TEMPERATURE_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.BODY_TEMPERATURE);
			try {
				VALIDATE_BODY_TEMPERATURE_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = helper.createInvariant(VALIDATE_BODY_TEMPERATURE_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP);
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		if (!EOCL_ENV.createQuery(VALIDATE_BODY_TEMPERATURE_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(bodyTemperature)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
